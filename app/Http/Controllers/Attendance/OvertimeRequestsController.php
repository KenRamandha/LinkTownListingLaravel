...
class OvertimeRequestsController extends Controller
{
    ...
    {
        try {
            if (!$r->user()->hasPermission('attendance.overtime:view')) return $this->fail('Forbidden', 403, 'FORBIDDEN');
            ...
            report($e);
            return $this->fail('Gagal memuat overtime requests', 500, 'SERVER_ERROR');
        }
    }
    public function store(Request $r)
    {
        try {
            if (!$r->user()->hasPermission('attendance.overtime:create')) return $this->fail('Forbidden', 403, 'FORBIDDEN');
            $r->validate([
            ...
            report($e);
            return $this->fail('Gagal membuat overtime request', 500, 'SERVER_ERROR');
        }
    }
    public function show(Request $r, string $id)
    {
        try {
            ...
            report($e);
            return $this->fail('Gagal memuat overtime request', 500, 'SERVER_ERROR');
        }
    }
    public function update(Request $r, string $id)
    {
        try {
            if (!$r->user()->hasPermission('attendance.overtime:update')) return $this->fail('Forbidden', 403, 'FORBIDDEN');
            ...
            report($e);
            return $this->fail('Gagal memperbarui overtime request', 500, 'SERVER_ERROR');
        }
    }
    public function approve(Request $r, string $id)
    {
        try {
            if (!$r->user()->hasPermission('attendance.overtime:approve')) return $this->fail('Forbidden', 403, 'FORBIDDEN');
            $exists = DB::table('overtime_requests')->where('id', $id)->exists();
            if (!$exists) return $this->fail('Overtime request tidak ditemukan', 404, 'NOT_FOUND');
            ...
            report($e);
            return $this->fail('Gagal menyetujui overtime request', 500, 'SERVER_ERROR');
        }
    }
    public function reject(Request $r, string $id)
    {
        try {
            ...
            report($e);
            return $this->fail('Gagal menolak overtime request', 500, 'SERVER_ERROR');
        }
    }
}
