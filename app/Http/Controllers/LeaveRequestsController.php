...
class LeaveRequestsController extends Controller
{
    ...
    {
        try { if(!$r->user()->hasPermission('attendance.leave:view')) return $this->fail('Forbidden',403,'FORBIDDEN');
        ...
        } catch(Throwable $e){ report($e); return $this->fail('Gagal memuat leave requests',500,'SERVER_ERROR'); }
    }
    public function store(Request $r)
    {
        try { if(!$r->user()->hasPermission('attendance.leave:create')) return $this->fail('Forbidden',403,'FORBIDDEN');
            $r->validate([
            ...
            DB::table('leave_requests')->insert([
                'id'=>$id,'user_id'=>$r->user_id,'leave_type_id'=>$r->leave_type_id,
        ...
        catch(Throwable $e){ report($e); return $this->fail('Gagal membuat leave request',500,'SERVER_ERROR'); }
    }
    public function show(Request $r, string $id)
    {
        ...
        } catch(Throwable $e){ report($e); return $this->fail('Gagal memuat leave request',500,'SERVER_ERROR'); }
    }
    public function update(Request $r, string $id)
    {
        try { if(!$r->user()->hasPermission('attendance.leave:update')) return $this->fail('Forbidden',403,'FORBIDDEN');
        ...
        catch(Throwable $e){ report($e); return $this->fail('Gagal memperbarui leave request',500,'SERVER_ERROR'); }
    }
    public function approve(Request $r, string $id)
    {
        try { if(!$r->user()->hasPermission('attendance.leave:approve')) return $this->fail('Forbidden',403,'FORBIDDEN');
            $exists=DB::table('leave_requests')->where('id',$id)->exists();
            if(!$exists) return $this->fail('Leave request tidak ditemukan',404,'NOT_FOUND');
        ...
        } catch(Throwable $e){ report($e); return $this->fail('Gagal menyetujui leave request',500,'SERVER_ERROR'); }
    }
    public function reject(Request $r, string $id)
    {
        ...
        } catch(Throwable $e){ report($e); return $this->fail('Gagal menolak leave request',500,'SERVER_ERROR'); }
    }
}
